...
Configurations *PtzManagement::getConfigurations()
{
    ...
    if(result != NULL){
        ...
        while(!item.isNull()){
            ...
            query->setQuery(result->nameSpace()+"./tt:DefaultAbsolutePanTiltPositionSpace/string()");
            query->evaluateTo(&value);
...
void PtzManagement::getConfiguration(Configuration *configuration)
{
    ...
    if(result != NULL){
        ...
        configuration->setDefaultAbsolutePanTiltPositionSpace(result->getValue("//tt:DefaultAbsolutePanTiltPositionSpace").trimmed());
